...
import {scaleSequential as d3scaleSequential,interpolateBlues as d3interpolateBlues,
  scaleBand as d3ScaleBand, scaleLinear as d3scaleLinear,
  axisBottom as d3axisBottom, axisLeft as d3axisLeft, event as d3event,
   min as d3min, max as d3max, select as d3select} from 'd3';
const https = require('https');
var d3 = require("d3");
var d3colorSequence = d3scaleSequential()
            .interpolator(d3interpolateBlues);
...
var createColor = (feature) => {
  if (covid_details == null){
    ...
    if (data.length){
      return d3colorSequence(parseInt(data[0].total_confirmed_cases));
    //   return new Fill({
    //     color: d3colorSequence(parseInt(data[0].total_confirmed_cases))
    //   });
    }
    return  'rgba(255, 255, 255, 0.6)';
  }
...
var map = new Map({
  controls: defaultControls().extend([
    new ZoomToExtent({
      extent: [
        7083572.2852,670199.8640, 10842024.260498, 4255931.279733
      ]
    })
  ]),
  layers: [vector],
...
https.get('https://api.data.gov.in/resource/cd08e47b-bd70-4efb-8ebc-589344934531?format=json&limit=all&api-key=579b464db66ec23bdd000001cdc3b564546246a772a26393094f5645',(res) => {
  ...
  res.on('end', function() {
    ...
    d3select('#confirmCases').html(total_cases);
    cured = covid_details.records.reduce((total, curr) =>{
      total += parseInt(curr.cured_discharged_migrated);
      return total;
    }, initial_val);
    d3select('#curedCases').html(cured);
    death = covid_details.records.reduce((total, curr) =>{
      total += parseInt(curr.death);
      return total;
    }, initial_val);
    d3select('#deathCases').html(death);
    let min = d3min(covid_details.records, d => parseInt(d.total_confirmed_cases));
    let max = d3max(covid_details.records, d => parseInt(d.total_confirmed_cases));
    d3colorSequence.domain([min, max+200]);
    console.log(covid_details);
...
var constructTable = function(data){
  ...
  let header = "<table class='table is-narrow is-bordered is-responsive'> \
  <thead> \
    <tr> \
    <th>Field</th> \
    <th>Value</th> \
...
function highestcaseChart(data){
  ...
  .domain([0, d3max(data, (d)=> d.total_confirmed_cases)])
  .range([ height, 0]);
  svg.append("g")
  .attr("class", "axis")
  ...
  .attr("fill", (d)=> {
          return d3colorSequence(d.total_confirmed_cases)})
...
var tooltipDiv = d3select('body').append('div')
.attr('class', 'tooltip')
.style('display', 'none');
...
function pieChart(rawData){
  ...
  svg.append("g")
      ...
      .join("path")
        ...
        .attr("d", arc)
      .append("title")
        .text(d => `${d.data.name_of_state_ut}: ${d.data.death}`)
...
function createDatatable(data){
  ...
  d3select('#tableView thead tr').html(th_rows);
  ...
  d3select('#tableView tbody').html(td_rows);
}