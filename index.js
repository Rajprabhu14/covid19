...
// var d3 = require("d3");
var colorSequence = d3.scaleSequential().interpolator(d3.interpolateBlues);
...
var filteredData = (name) =>
  covid_details.records.filter((d) => d.name_of_state_ut == name);
...
var constructTable = function (data) {
  ...
  let mobile_body = `<tbody><tr> \
    ...
    <td>LastUpdate</td> \
    <td>${
      data.date_time.split(" ")[0] + "<br/>" + data.date_time.split(" ")[1]
    }</td> \
    </tr> </tbody></table>`;
...
https
  .get(
    "https://api.data.gov.in/resource/cd08e47b-bd70-4efb-8ebc-589344934531?format=json&limit=all&api-key=579b464db66ec23bdd000001cdc3b564546246a772a26393094f5645",
    (res) => {
      ...
      res.on("end", function () {
        ...
        let min = d3.min(covid_details.records, (d) =>
          parseInt(d.total_confirmed_cases)
        );
        let max = d3.max(covid_details.records, (d) =>
          parseInt(d.total_confirmed_cases)
        );
        ...
        var sorted_data = covid_details.records.sort((a, b) =>
          parseInt(a.s_no) < parseInt(b.s_no) ? 1 : -1
        );
        covid_details.records = sorted_data.reverse();
        // top for bar chart most cases
        var top5Cases = covid_details.records
          .sort((a, b) =>
            parseInt(a.total_confirmed_cases) <
            parseInt(b.total_confirmed_cases)
              ? 1
              : -1
          )
          .slice(0, 5);
...
function pieChart(rawData) {
  ...
  rawData.forEach((d) => {
    d.death = +d.death;
    // groupedData[d.death] = [...(d[name_of_state_ut] || []), d.name_of_state_ut];
  });
  let data = rawData.filter((d) => d.death != 0);
  data.sort((a, b) => (parseInt(a.death) < parseInt(b.death) ? 1 : -1));
  data = data.filter((currentValue, index, arr) => index < 10);
  let color = d3
    .scaleOrdinal()
...
window.addEventListener("resize", function (event) {
  map
    .getView()
    .fit([7083572.2852, 670199.864, 10842024.260498, 4255931.279733], {
      constrainResolution: true,
    });
  var top5Cases = covid_details.records
    .sort((a, b) =>
      parseInt(a.total_confirmed_cases) < parseInt(b.total_confirmed_cases)
        ? 1
        : -1
    )
    .slice(0, 5);
